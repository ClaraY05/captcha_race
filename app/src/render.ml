...
let clear () =
  ...
  Graphics.fill_rect 0 0 Layout.window_width Layout.window_height
;;
...
let center_x = Layout.window_width / 2
...
let draw_playing runner ~now =
  (* HUD strip along the top, above [Layout.play_bounds]. *)
  let hud_y = Layout.window_height - 40 in
  Graphics.set_color Graphics.black;
  ...
  Graphics.lineto Layout.window_width play_h;
  match Game_runner.current runner with
  | None -> ()
